...
工程处于开发中。后端已具备新闻采集与事件聚合、知识材料导入、AI 分析校验、热点与晨报生成及读取、带引用校验的问答适配和反馈 API。Android 首页为日历模式，默认展示今日推荐并可按日期查看历史热点和晨报；晨报会提前缓存，可从首页播放，也可按闹钟设置在停止响铃后通过系统中文 TTS 自动或手动播放。Debug APK 已在模拟器验证原有一次性闹钟的息屏触发与停止，新增晨报播报和真机可靠性仍待验收。完整进度与未通过的验收项见 [开发进度](docs/PROGRESS.md)。
