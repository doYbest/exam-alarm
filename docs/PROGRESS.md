...
更新于 2026-09-24。需求基线为根目录 `DEVELOPMENT_SPEC.md`。本文件记录代码状态；没有真机或真实数据库的检查不标记通过。
...
| M6 Android 热点 | Compose 日历首页、按日期查看历史热点与晨报、今日推荐、详情、Room 缓存、WorkManager 同步、离线详情和开发健康检查页已写入，Debug APK 构建通过 | 日历与历史内容的模拟器、真机交互验收 |
| M7 闹钟与 TTS | Room 闹钟与晨报缓存模型及迁移、晨报提前同步、当天/24 小时回退选择、系统中文 TTS、通知暂停/继续/停止、首页手动播放、闹钟停止后自动或手动播放均已写入；模拟器上原有一次性闹钟链路已验证 | 新增晨报播报的模拟器和真机听感、音频焦点、中文语音数据缺失、断网回退验收；真机锁屏、Doze、重启恢复与权限降级验收 |
...
晨报/日历主体改动后 Gradle testDebugUnitTest、lintDebug、assembleDebug：通过；新增的 2 个播报文本单测因本地 Gradle daemon 后续无响应，仍待复跑
模拟器一次性闹钟：09:35 精确 RTC_WAKEUP 已登记；息屏且应用进程退出后触发锁屏响铃界面；停止后服务退出，测试闹钟已删除。
```
