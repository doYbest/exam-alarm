...
更新于 2026-09-22。需求基线为根目录 `DEVELOPMENT_SPEC.md`。本文件记录代码状态；没有真机或真实数据库的检查不标记通过。
...
| M7 闹钟与 TTS | Room 闹钟模型与迁移、时间计算、闹钟增删改启停、AlarmManager 注册、系统事件恢复、响铃与稍后提醒代码已写入；模拟器上一次性闹钟在息屏且进程退出后准时触发，锁屏响铃界面出现，停止操作正常 | 真机锁屏、Doze、重启恢复、稍后提醒与权限降级验收；晨报 TTS 尚未实现 |
